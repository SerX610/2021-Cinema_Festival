...
/* For each film, we will need to know its numeric identifier (index) and its
restrictions with the other ones. */
struct Film_info {
  int idx;
  vector<bool> restrictions;
};

// We will use this structure to store the solution plans and their info.
struct Solution {
  vector<Day> plan;
  int cost;
  int days;
...
/* Given the films' informations, it will make the plan in a number d of days
...
void generate_planning(const vector<Film_info> &films_info, vector<Day> &plan,
  ...
  for (int i = 1; i < f; ++i) {
    /* For all films, we pick the following most restricted film index, which
...
int neighbour_restrictions(Solution &neighbour,
  ...
  while (old_day == new_day) {
    ...
    // A non empty day is needed to pick a film from;
    while (neighbour.plan[old_day].size() == 0)
      old_day = rand() % neighbour.days;
    new_day = rand() % (neighbour.days + 1);
    // A non complete day is needed to play a film;
...
int main(int argc, char **argv) {
  ...
  so that a different one is generated every iteration before applying the
  metaheuristics. */
  ...
  while (true) {
    simulated_annealing(films_titles, films_info, cinemas, current_plan,
                        optimal, d, 0.99);
