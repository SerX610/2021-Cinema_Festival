...
void exhaustive_search_planning(vector<Film> &films,
                                const vector<string> &cinemas,
                                vector<Day> &current_plan,
                                vector<Day> &best_plan, int k, int d) {
  if (k == f) {
    min_d = d;
    ...
    for (int i = 0; i < f; ++i) {
      if (not films[i].projected and
          not restricted(films[i], current_plan[d - 1])) {
        current_plan[d - 1].push_back(i);
        films[i].projected = true;
        if (current_plan[d - 1].size() == c and k + 1 < f)
          exhaustive_search_planning(films, cinemas, current_plan, best_plan,
                                     k + 1, d + 1);
        else
          exhaustive_search_planning(films, cinemas, current_plan, best_plan,
                                     k + 1, d);
